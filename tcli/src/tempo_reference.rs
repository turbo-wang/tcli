...
tcli — same command names where applicable; implementation differs:
  tcli wallet login | logout — OAuth2 device flow (QR PNG under ~/.openclaw/workspace/tcli-login/<session>/ + path in JSON); token in ~/.tcli/wallet/oauth.json
...
Configuration:
  ...
  Login: stdout prints one JSON line with qr_png_path / auth_url; a background process polls the OAuth token endpoint until authorized.
